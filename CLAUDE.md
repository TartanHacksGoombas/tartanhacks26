...
```
tartanhacks26/
├── backend/             # Express API + ML pipeline + data
│   ├── src/             # TypeScript API code
│   ├── pipeline/        # Data collection (18 scripts) + ML pipeline (7 scripts) + orchestrator
...
### Pittsburgh-Wide Data Collection Pipeline (`backend/pipeline/`)

All scripts are standalone and output CSVs to `backend/data/` with a `data_source` column.
...
- **build_labels.py** — Detects 126 storm events from historical weather (snowfall >= 1cm, consecutive days grouped, +2 day extension). Expands each storm into per-day rows via `expand_storm_days()` with temporal features (`day_offset`, `days_since_peak_snowfall`, `is_post_storm`, `cumulative_snowfall_cm`, `daily_snowfall_cm`, `cumulative_thaw_degree_hours`). Labels each of 17,549 unique segments per (storm, day) using 4 proxy sources: 311 snow complaints (200m, `query_ball_point`, assigned to specific days), winter crashes (200m, monthly proportional allocation distributed across storm days), plow activity gaps (100m, binary replicated across days, 3/126 storms have data), DOMI full closures (200m, `query_ball_point`, assigned to overlapping days). Composite risk score via log-scaled min-max normalization (zeros → 0) + weighted sum (0.45 × 311 + 0.25 × crash + 0.15 × plow + 0.15 × DOMI). Also outputs label variants: `label_any_incident` (binary), `label_top_1pct`, `label_top_5pct`. Outputs `storm_events.csv` (126 storms), `storm_days.csv` (~500-630 storm-day rows), and `storm_labels.csv` (~10M rows at per-day granularity).
- **build_training_data.py** — Encodes 31 static features (ordinal encoding for road class/pavement/highway type, log1p for count features, -1 sentinel for missing PennDOT, bool-to-int for flags, `incline_pct` parsed from OSM strings, `bridge_age_years`). Joins with 6 storm weather features, 6 temporal features (`day_offset`, `days_since_peak_snowfall`, `is_post_storm`, `cumulative_snowfall_cm`, `cumulative_thaw_degree_hours`, `daily_snowfall_cm`), 5 weather × segment interaction features (`snow_x_steep_slope`, `snow_x_incline`, `temp_x_bridge`, `wind_x_roadwidth`, `snow_x_elevation`), and 3 temporal × segment interaction features (`thaw_x_steep_slope`, `post_storm_x_elevation`, `day_offset_x_snow`). Auto-detects `storm_days.csv` for per-day granularity; falls back to per-storm if not available. Carries label variants. Writes `training_matrix.parquet` via chunked processing with PyArrow to avoid OOM. Deduplicates segments by objectid.
...
- **predict.py** — Prefers ranker model (falls back to regression). Two input modes: NWS forecast JSON parsing or CLI args (`--snowfall_cm`, `--min_temp_c`, `--max_wind_kmh`, `--duration_days`). Supports lingering snow via `--recent_snowfall_cm`, `--hours_since_snow`, `--temp_avg_since_snow_c` — uses temperature-dependent exponential decay + plowing linear decay to estimate effective snow on ground. Temporal features (`--day_offset`, `--days_since_peak`, `--is_post_storm`, `--cumulative_thaw_degree_hours`) are auto-detected from model metadata. Deduplicates segments by `objectid` before encoding. Broadcasts weather features and computes interaction features. Calibrates via sigmoid ranking + weather-severity power transform (light snow compresses scores toward 0, blizzard spreads them). Categorizes (very_low/low/moderate/high/very_high). Outputs `predictions_latest.csv` and `predictions_latest.json` (GeoJSON).
...
- Lingering snow (20cm fell 18h ago, -5C): effective snow computed via thermal decay + plow decay, produces meaningful risk even with --snowfall_cm 0
...
# Lingering snow: 20cm fell yesterday, no new snow, roads still buried:
python pipeline/predict.py --snowfall_cm 0 --recent_snowfall_cm 20 --hours_since_snow 18 --min_temp_c -5

# Warm melt scenario (less lingering risk):
python pipeline/predict.py --snowfall_cm 0 --recent_snowfall_cm 20 --hours_since_snow 18 --min_temp_c 3 --temp_avg_since_snow_c 3
```
...
- `storm_labels.csv` now has per-day granularity (~10M rows) with columns: `objectid`, `storm_id`, `day_offset`, `storm_311_count`, `storm_crash_score`, `storm_plow_gap`, `storm_domi_closure`, `risk_score`, `label_any_incident`, `label_top_1pct`, `label_top_5pct`. `build_training_data.py` reads it in 500K-row chunks to avoid OOM.
- `storm_days.csv` contains ~500-630 rows with temporal features per (storm_id, day_offset): `daily_snowfall_cm`, `cumulative_snowfall_cm`, `days_since_peak_snowfall`, `is_post_storm`, `cumulative_thaw_degree_hours`.
- `training_matrix.parquet` uses float32 and chunked Parquet writing via PyArrow for memory efficiency. Contains ~57 columns: objectid, storm_id, day_offset, risk_score, 3 label variants, 31 static features, 6 weather features, 6 temporal features, 5 weather interaction features, 3 temporal interaction features, season.
...
- All pipeline scripts resolve paths via `DATA_DIR` relative to `backend/`
...
- Cache: skip re-fetch if output CSV already exists in `backend/data/`
...
- Generated data files go in `backend/data/` (gitignored)
